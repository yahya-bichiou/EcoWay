...
public function indexp(
    ...
    return $this->render('backend/products.html.twig', [
        ...
        'produitForm' => $this->createForm(Produit1Type::class, new Produit())->createView(),
        'categorieForm' => $this->createForm(CategorieType::class, new Categorie())->createView(),
    ]);
}
...
{
    if (!$produit) {
        // Si le produit n'est pas trouvé, rediriger ou afficher une erreur
        return $this->redirectToRoute('back_products');
    }

    // Créer le formulaire pour modifier le produit
    $form = $this->createForm(Produit1Type::class, $produit);
    $form->handleRequest($request);

    if ($form->isSubmitted() && $form->isValid()) {
        $entityManager->flush(); // Appliquer les modifications
        return $this->redirectToRoute('back_products'); // Rediriger vers la liste des produits
    }
